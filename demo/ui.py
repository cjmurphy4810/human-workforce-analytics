...
def render_demo_sidebar(active_page: str) -> str:
    with st.sidebar:
        ...
        st.button(DEMO_CHANNEL_NAME, type="primary", width="stretch")
        for name in DISABLED_CHANNELS:
            st.button(name, disabled=True, width="stretch")
...
def render_database_maintenance() -> None:
    """Render one public-safe message for every fixture access failure."""
    st.warning(DATABASE_MAINTENANCE_MESSAGE, icon="🛠️")
