...
def _seed_db(conn: sqlite3.Connection) -> None:
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS ci_video_scores (
            ...
            promotion_ratio REAL, PRIMARY KEY (scored_at, video_id)
        );
    ...
    conn.execute(
        ...
        "VALUES (?,?,?,?,?,?,?)",
        ("2026-06-29", "v1", 1000, 5000, 300, 50, 10),
    )
    conn.execute(
        "INSERT INTO daily_video_metrics(metric_date, video_id, views, "
        "estimated_minutes_watched, average_view_duration, likes, subscribers_gained) "
        "VALUES (?,?,?,?,?,?,?)",
        ("2026-06-29", "v2", 100, 200, 200, 5, 3),
    )
    conn.commit()
...
def test_score_videos_returns_one_score_per_video():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with sqlite3.connect(str(db_path)) as conn:
            _seed_db(conn)
        scores = score_videos(db_path, date(2026, 6, 29))
        assert len(scores) == 2
